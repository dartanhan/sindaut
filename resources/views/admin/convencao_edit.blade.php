...
@section('header_title', 'Editar Convenção')
@section('header_subtitle', 'Edite as informações da Convenção Coletiva (CCT)')
...
<div class="max-w-5xl mx-auto">
    ...
    <form id="convencaoForm" action="{{ route('convencao.update', $convencao->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8 pb-20">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div class="space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="titulo" id="titulo" value="{{ old('titulo', $convencao->titulo_cct) }}" required maxlength="500" placeholder="Ex: Convenção Coletiva de Trabalho Comércio 2023/2024" class="w-full bg-white border border-slate-200 rounded-2xl p-4 font-bold text-slate-900 focus:border-blue-600 focus:ring-0 transition">
                </div>
                
                <!-- Vigencia / Ano -->
                <div class="space-y-2">
                    <label class="text-xs font-black text-slate-400 uppercase tracking-widest">Vigência (Ex: 2023/2024)</label>
                    <input type="text" name="data_cct" id="data_cct" value="{{ old('data_cct', $convencao->data_cct) }}" required maxlength="10" placeholder="Ex: 2023/2024" class="w-full bg-white border border-slate-200 rounded-2xl p-4 font-bold text-slate-900 focus:border-blue-600 focus:ring-0 transition">
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <select name="status" class="w-full bg-white border border-slate-200 rounded-2xl p-4 text-xs font-bold uppercase tracking-widest focus:border-blue-600 focus:ring-0 transition">
                        <option value="1" {{ old('status', $convencao->status) == '1' ? 'selected' : '' }}>Ativo / Publicado</option>
                        <option value="0" {{ old('status', $convencao->status) == '0' ? 'selected' : '' }}>Inativo / Rascunho</option>
                    </select>
                </div>
            </div>
            ...
            <div class="space-y-2 flex flex-col justify-end">
                <label class="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">Arquivo PDF da Convenção (Opcional se já enviado)</label>
                
                <div id="fileUploadContainer" 
                     class="relative h-full min-h-[220px] bg-blue-50/10 border-2 border-solid border-blue-600 rounded-[2.5rem] flex flex-col items-center justify-center p-8 text-center hover:border-blue-600 transition group cursor-pointer overflow-hidden">
                    
                    <input type="file" name="image" id="fileInput" accept="application/pdf" class="absolute inset-0 opacity-0 cursor-pointer z-20 w-full h-full" onchange="previewFile(event)">
                    
                    <div id="uploadPlaceholder" class="space-y-3 group-hover:scale-105 transition relative z-10">
                        <div class="w-20 h-20 bg-slate-50 text-blue-600 rounded-3xl flex items-center justify-center mx-auto shadow-sm">
                            <i data-lucide="file-text" class="w-10 h-10 text-blue-600"></i>
                        </div>
                        <p class="text-[10px] font-black text-blue-600 uppercase tracking-widest" id="fileNamePlaceholder">
                            @if($filePath)
                                {{ $filePath }} (Já cadastrado - Clique para alterar)
                            @else
                                Clique ou arraste para selecionar o PDF
                            @endif
                        </p>
                    </div>
                </div>
