...
@section('title', 'Editar Conteúdo Jurídico')
@section('header_title', 'Editar Conteúdo Jurídico')
@section('header_subtitle', 'Edite as orientações e documentos do departamento jurídico')
...
<div class="max-w-5xl mx-auto">
    ...
    <form id="depjuridicoForm" action="{{ route('depjuridico.update', $depjuridico->id) }}" method="POST" class="space-y-8 pb-20">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-2 max-w-xs">
            ...
            <select name="status" class="w-full bg-white border border-slate-200 rounded-2xl p-4 text-xs font-bold uppercase tracking-widest focus:border-blue-600 focus:ring-0 transition">
                <option value="1" {{ old('status', $depjuridico->status) == '1' ? 'selected' : '' }}>Ativo / Publicado</option>
                <option value="0" {{ old('status', $depjuridico->status) == '0' ? 'selected' : '' }}>Inativo / Rascunho</option>
            </select>
        </div>
        ...
        <div class="space-y-4">
            ...
            <div class="bg-white rounded-[2.5rem] overflow-hidden border border-slate-200 shadow-sm">
                <textarea class="tinymce_editor" name="tinymce_editor" id="tinymce_editor">{{ old('tinymce_editor', $depjuridico->conteudo) }}</textarea>
            </div>
        </div>
    </form>
</div>
