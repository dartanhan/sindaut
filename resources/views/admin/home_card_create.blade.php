...
@section('title', 'Novo Card da Home')
@section('header_title', 'Novo Card da Home')
@section('header_subtitle', 'Crie um novo card para a página inicial')
...
<div class="flex gap-4">
    ...
    <button form="homeCardForm" type="submit" id="btnSubmit" class="bg-blue-600 hover:bg-slate-900 text-white font-black px-8 py-3 rounded-2xl transition shadow-xl shadow-blue-600/20 text-sm flex items-center gap-2">
        <span id="btnText">PUBLICAR CARD</span>
...
<div class="max-w-5xl mx-auto">
    ...
    <form id="homeCardForm" action="{{ route('home-card.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8 pb-20">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div class="space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="titulo" id="titulo" value="{{ old('titulo') }}" required maxlength="80" placeholder="Ex: Benefício Social" class="w-full bg-white border border-slate-200 rounded-2xl p-4 font-bold text-slate-900 focus:border-blue-600 focus:ring-0 transition">
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="number" name="ordem" value="{{ old('ordem', 0) }}" class="w-full bg-white border border-slate-200 rounded-2xl p-4 font-bold text-slate-900 focus:border-blue-600 focus:ring-0 transition">
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-2 flex flex-col">
                ...
                @php
                    $initialImage = null;
                    if (old('idImagemDestaque')) {
                        $initialImage = $images->firstWhere('id', old('idImagemDestaque'));
                    }
                @endphp
                ...
                <div id="imagePreviewContainer" 
                     ...
                     style="@if($initialImage) background-image: url('{{ asset('storage/posts/files/'.$initialImage->path) }}') @endif">
                    ...
                    <div id="previewPlaceholder" class="space-y-3 group-hover:scale-110 transition relative z-10 @if($initialImage) bg-white/80 p-6 rounded-2xl backdrop-blur-sm shadow-xl @endif">
