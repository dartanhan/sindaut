...
@section('header_title', 'Nova Notícia')
@section('header_subtitle', 'Crie um novo conteúdo para o portal')
...
<div class="flex gap-4">
    ...
    <button form="noticiaForm" type="submit" id="btnSubmit" class="bg-blue-600 hover:bg-slate-900 text-white font-black px-8 py-3 rounded-2xl transition shadow-xl shadow-blue-600/20 text-sm flex items-center gap-2">
        <span id="btnText">PUBLICAR NOTÍCIA</span>
...
<div class="max-w-5xl mx-auto">
    ...
    <form id="noticiaForm" action="{{ route('noticia.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8 pb-20">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div class="space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="titulo" id="titulo" value="{{ old('titulo') }}" required maxlength="80" placeholder="Ex: Novo benefício disponível para associados" class="w-full bg-white border border-slate-200 rounded-2xl p-4 font-bold text-slate-900 focus:border-blue-600 focus:ring-0 transition">
                </div>
                
                <!-- Subtitle -->
                <div class="space-y-2">
                    <label class="text-xs font-black text-slate-400 uppercase tracking-widest">Subtítulo (Resumo - Máximo 250 Caracteres)</label>
                    <textarea id="subtitulo" name="subtitulo" rows="3" maxlength="250" placeholder="Breve resumo da notícia..." class="w-full bg-white border border-slate-200 rounded-2xl p-4 font-bold text-slate-900 focus:border-blue-600 focus:ring-0 transition">{{ old('subtitulo') }}</textarea>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <!-- Date -->
                    <div class="space-y-2">
                        <label class="text-xs font-black text-slate-400 uppercase tracking-widest">Data de Cadastro (Opcional)</label>
                        <input type="datetime-local" name="data_cadastro" value="{{ old('data_cadastro') }}" class="w-full bg-white border border-slate-200 rounded-2xl p-4 font-bold text-slate-900 focus:border-blue-600 focus:ring-0 transition">
                    </div>
                </div>
