...
@section('header_subtitle', 'Cadastre um novo usuário com acesso administrativo')
...
<div class="flex gap-4">
    ...
    <button form="usuarioForm" type="submit" id="btnSubmit" class="bg-blue-600 hover:bg-slate-900 text-white font-black px-8 py-3 rounded-2xl transition shadow-xl shadow-blue-600/20 text-sm flex items-center gap-2">
        <span id="btnText">SALVAR USUÁRIO</span>
...
<div class="max-w-3xl mx-auto">
    ...
    <form id="usuarioForm" action="{{ route('usuario.store') }}" method="POST" class="bg-white p-8 rounded-[2.5rem] border border-slate-200/80 shadow-sm space-y-6">
        @csrf
        ...
        <div class="space-y-2">
            ...
            <input type="email" name="email" value="{{ old('email') }}" placeholder="Ex: joao@sindaut.org.br" 
                   class="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 text-sm font-bold text-slate-700 placeholder-slate-400 focus:bg-white focus:border-blue-600 focus:ring-0 transition" required>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="space-y-2">
                <label class="text-xs font-black text-slate-400 uppercase tracking-widest">Senha</label>
                <input type="password" name="password" placeholder="Mínimo 6 caracteres" 
                       class="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 text-sm font-bold text-slate-700 placeholder-slate-400 focus:bg-white focus:border-blue-600 focus:ring-0 transition" required>
            </div>
            
            <div class="space-y-2">
                <label class="text-xs font-black text-slate-400 uppercase tracking-widest">Confirmar Senha</label>
