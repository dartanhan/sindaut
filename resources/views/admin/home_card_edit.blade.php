...
@section('title', 'Editar Card da Home')
@section('header_title', 'Editar Card da Home')
@section('header_subtitle', 'Atualize as informações do card selecionado')

@section('header_actions')
<div class="flex gap-4">
    <a href="{{ route('home-card.index') }}" class="text-slate-400 hover:text-slate-900 font-bold px-6 py-3 transition text-sm flex items-center">CANCELAR</a>
    <button form="homeCardForm" type="submit" id="btnSubmit" class="bg-blue-600 hover:bg-slate-900 text-white font-black px-8 py-3 rounded-2xl transition shadow-xl shadow-blue-600/20 text-sm flex items-center gap-2">
...
<div class="max-w-5xl mx-auto">
    ...
    <form id="homeCardForm" action="{{ route('home-card.update', $card->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8 pb-20">
        @csrf
        @method('PUT')
        <input type="hidden" name="idImagemDestaque" id="idImagemDestaque" value="{{ old('idImagemDestaque', $card->imagem_id) }}">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div class="space-y-6">
                <!-- Title -->
                <div class="space-y-2">
                    <label class="text-xs font-black text-slate-400 uppercase tracking-widest">Título do Card (Máximo 80 Caracteres)</label>
                    <input type="text" name="titulo" id="titulo" value="{{ old('titulo', $card->titulo) }}" required maxlength="80" placeholder="Ex: Benefício Social" class="w-full bg-white border border-slate-200 rounded-2xl p-4 font-bold text-slate-900 focus:border-blue-600 focus:ring-0 transition">
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="status" class="w-full bg-white border border-slate-200 rounded-2xl p-4 text-xs font-bold uppercase tracking-widest focus:border-blue-600 focus:ring-0 transition">
                            <option value="1" {{ old('status', $card->status) == '1' ? 'selected' : '' }}>Ativo</option>
                            <option value="0" {{ old('status', $card->status) == '0' ? 'selected' : '' }}>Inativo</option>
                        </select>
                    </div>

                    <!-- Order -->
                    <div class="space-y-2">
                        <label class="text-xs font-black text-slate-400 uppercase tracking-widest">Ordem / Posição (Ex: 1, 2, 3)</label>
                        <input type="number" name="ordem" value="{{ old('ordem', $card->ordem) }}" class="w-full bg-white border border-slate-200 rounded-2xl p-4 font-bold text-slate-900 focus:border-blue-600 focus:ring-0 transition">
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-2 flex flex-col">
                <div class="flex justify-between items-center mb-1">
                    <label class="text-xs font-black text-slate-400 uppercase tracking-widest">Imagem do Card</label>
                ...
                @php
                    $selectedImage = null;
                    $idDestaque = old('idImagemDestaque', $card->imagem_id);
                    if ($idDestaque) {
        ...
        <div class="space-y-4">
            <div class="flex justify-between items-center">
                <label class="text-xs font-black text-slate-400 uppercase tracking-widest">Conteúdo (Exibido ao clicar no Card)</label>
            </div>
            <div class="bg-white rounded-[2.5rem] overflow-hidden border border-slate-200 shadow-sm">
                <textarea class="tinymce_editor" name="tinymce_editor" id="tinymce_editor">{{ old('tinymce_editor', $card->conteudo) }}</textarea>
            </div>
        </div>
    </form>
</div>
...
<div id="imagemModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-950/60 backdrop-blur-sm p-4">
    <div class="bg-white rounded-[2rem] shadow-2xl border border-slate-100 max-w-xl w-full max-h-[80vh] flex flex-col overflow-hidden">
        <div class="bg-blue-600 text-white px-8 py-6 flex justify-between items-center">
            <h5 class="font-black text-lg uppercase tracking-tight">Selecione a Imagem</h5>
            ...
            <div class="grid grid-cols-3 gap-4 product-list-gallery">
                @foreach($images as $image)
                    <div class="group relative bg-white border border-slate-100 rounded-2xl overflow-hidden shadow-sm hover:shadow-md hover:border-blue-600 transition cursor-pointer product-card-gallery">
                        <img src="{{ asset('storage/posts/files/'.$image->path) }}" data-id="{{ $image->id }}" class="w-full h-24 object-cover imagem-selecao" title="Selecionar para Card">
...
<script>
    ...
    if (homeCardForm) {
        homeCardForm.addEventListener('submit', function(event) {
            ...
            if (titulo.trim() === "") {
                Swal.fire({
                    title: 'Atenção!',
                    text: 'O Título do card deve ser informado!',
                    icon: 'info',
                    confirmButtonText: 'OK'
                });
                return;
            ...
            if (tinymceContent.trim() === "") {
                Swal.fire({
                    title: 'Atenção!',
                    text: 'O Conteúdo do Card está vazio!',
                    icon: 'info',
                    confirmButtonText: 'OK'
                });
                return;
