...
<div class="space-y-8">
    ...
    <div class="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden">
        <div class="p-8 border-b border-slate-100 flex items-center justify-between">
            ...
            <span class="text-xs font-black text-slate-400 uppercase tracking-widest">{{ empty($beneficio) ? 'Nenhum' : '1' }} cadastrado</span>
        </div>
...
<div id="exampleModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-950/60 backdrop-blur-sm p-4 overflow-y-auto">
    <div class="bg-white rounded-[2rem] shadow-2xl border border-slate-100 max-w-4xl w-full max-h-[90vh] flex flex-col overflow-hidden">
        <form method="POST" action="{{ route('beneficio.store') }}" name="uploadForm" id="uploadForm" class="flex flex-col h-full overflow-hidden">
            @csrf
            <!-- Header -->
            <div class="bg-slate-900 text-white px-8 py-6 flex justify-between items-center">
                <h5 class="font-black text-lg uppercase tracking-tight">Cadastrar Conteúdo de Benefícios</h5>
            ...
            <div class="p-8 overflow-y-auto space-y-6 flex-1 text-slate-700">
                <div class="space-y-2">
                    <label class="text-xs font-black uppercase text-slate-400 tracking-wider">Descrição dos Benefícios</label>
...
<div id="modalConteudo" class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-950/60 backdrop-blur-sm p-4">
    <div class="bg-white rounded-[2rem] shadow-2xl border border-slate-100 max-w-3xl w-full max-h-[85vh] flex flex-col overflow-hidden">
        <!-- Header -->
        <div class="bg-slate-900 text-white px-8 py-6 flex justify-between items-center">
            <h5 class="font-black text-lg uppercase tracking-tight">Visualizar Benefícios</h5>
...
<div id="editModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-950/60 backdrop-blur-sm p-4 overflow-y-auto">
    <div class="bg-white rounded-[2rem] shadow-2xl border border-slate-100 max-w-4xl w-full max-h-[90vh] flex flex-col overflow-hidden">
        <form method="POST" action="{{ route('beneficio.update', $beneficio->id) }}" name="uploadForm" id="uploadForm" class="flex flex-col h-full overflow-hidden">
            @csrf
            @method('put')
            <!-- Header -->
            <div class="bg-slate-900 text-white px-8 py-6 flex justify-between items-center">
                <h5 class="font-black text-lg uppercase tracking-tight">Editar Benefícios</h5>
